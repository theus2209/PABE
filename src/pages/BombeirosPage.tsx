...
import { AddBombeiroDialog } from '@/components/AddBombeiroDialog';
import { ConfirmDialog } from '@/components/ConfirmDialog';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { ArrowLeft, RefreshCw, Plus, Users, Phone, Flame, Search, ChevronUp, ChevronDown } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import type { BombeiroContato } from '@/types';

export function BombeirosPage() {
  const navigate = useNavigate();
  const { contatos, loading, searchTerm, setSearchTerm, addContato, deleteContato, reorderContatos, refetch } = useBombeiros();
  ...
  const handleAddContato = async (
    ...
    return await addContato(postoGraduacao, nomeGuerra, telefone);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-600 via-red-700 to-red-800">
      ...
      <div className="bg-gradient-to-r from-red-700 to-red-900 text-white p-6 shadow-lg">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-3xl font-bold">Bombeiros</h1>
        </div>
      </div>
                      ...
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-sm font-bold text-white mb-0">
                            {contato.posto_graduacao} {contato.nome_guerra}
                          </h3>
